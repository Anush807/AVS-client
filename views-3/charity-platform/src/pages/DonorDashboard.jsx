  ...
  const fetchData = async () => {
    try {
      const [campaignsRes, donationsRes, leadersRes] = await Promise.all([
        ...
        donationAPI.getMyDonations().catch(err => {
          console.error('Donations API Error:', err);
          return { data: [] };
        }),
        dashboardAPI.getTopDonors().catch(err => {
      ...
      setTopDonors(Array.isArray(leadersRes.data) ? leadersRes.data : []);
    } catch (error) {
...
Issued: ${new Date(receipt.issuedAt).toLocaleString()}
  ...
  // Fix: Use user.id (from localStorage) to find rank
  const userRank = topDonors.findIndex(d => d._id === user.id || d._id.toString() === user.id) + 1;
            ...
            <div>
              ...
              {loading ? (
                ...
                <div className="card text-center py-12">
                  ...
                  <p className="text-neutral-600 mb-2">No active campaigns available</p>
                  <p className="text-sm text-neutral-500">Check back later for new campaigns to support</p>
                </div>
              )}
            </div>
          )}
                  ...
                  {topDonors.map((donor, index) => (
                    <div
                      ...
                      className={`flex items-center justify-between p-4 rounded-lg transition-all duration-200 ${
                        (donor._id === user.id || donor._id.toString() === user.id)
...
const DonateModal = ({ campaign, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update user data in localStorage with new points and badge from response
      if (response.data) {
        const currentUser = JSON.parse(localStorage.getItem('user'));
        const updatedUser = {
          ...currentUser,
          points: response.data.totalPoints || currentUser.points,
          badge: response.data.badge || currentUser.badge,
        };
